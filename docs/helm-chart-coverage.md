...
| `serviceAccount.create/name/awsRoleArn` | Hardcoded `true` / `"n8n-enterprise"` / wired to the module's Pod Identity IAM role (see [docs/pod-identity.md](./pod-identity.md)) |
...
| `redis.tls` | Hardcoded `false`; not exposed as a variable today |
