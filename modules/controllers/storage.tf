...
#
# Gated separately from create_eks (the root module's toggle for the cluster
# itself), on its own create_ebs_csi toggle: a shared cluster already hosting
# other workloads very likely already runs a CSI driver, and installing a
# second aws-ebs-csi-driver addon on one that already has one fails outright
# rather than degrading gracefully. Defaults to true so every existing
# deployment (which has always gotten this unconditionally) plans unchanged.
...
resource "aws_eks_addon" "ebs_csi" {
  ...
  # Policy attachment: the role must never be live without
  # AmazonEBSCSIDriverPolicy. The node group and Pod Identity agent
  # dependencies live in the root module, which depends_on this whole
  # submodule before creating anything that assumes controllers are ready.
  depends_on = [
