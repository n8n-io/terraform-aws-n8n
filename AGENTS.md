...
- **ACM certificate + DNS record** automated via Route 53 (`route53_zone_id`) or
  Cloudflare (`cloudflare_zone_id`), or BYO pre-validated `certificate_arn`.
...
include the VPC live in [`examples/complete/`](./examples/complete/) (Route 53)
and [`examples/cloudflare/`](./examples/cloudflare/) (Cloudflare DNS).
...
| `eks.tf` / `database.tf` / `redis.tf` / `s3.tf` / `iam.tf` / `dns.tf` / `n8n.tf` / `controllers.tf` / `keda.tf` / `scaling.tf` | One file per logical concern. `dns.tf` owns **all** DNS automation — both Route 53 and Cloudflare paths. Do **not** split by provider. |
...
| `examples/complete/`              | End-to-end runnable example using Route 53 DNS, including the VPC. |
