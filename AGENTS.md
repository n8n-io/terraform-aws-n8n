...
The module **expects a pre-existing VPC**. Complete reference deployments that
include the VPC live in [`examples/small/`](./examples/small/) (Route 53),
[`examples/cloudflare/`](./examples/cloudflare/) (Cloudflare DNS), and
...
| `examples/small/`                 | End-to-end runnable example using Route 53 DNS, including the VPC. |
| `examples/cloudflare/`            | End-to-end runnable example using Cloudflare DNS, including the VPC. |
| `examples/godaddy/`               | End-to-end runnable example using GoDaddy DNS, including the VPC.    |
...
- **`terraform validate`** against the module root *and* `examples/small/`.
...
- `examples/small/tests/defaults.tftest.hcl` exercises the example end-to-end
...
- `examples/small/README.md` documents the runnable example.
...
# Repeat under each example.
cd examples/small && terraform init -backend=false && terraform validate && terraform test -verbose
cd examples/cloudflare && terraform init -backend=false && terraform validate && terraform test -verbose
cd examples/godaddy && terraform init -backend=false && terraform validate && terraform test -verbose
```
