...
Each example wires the `kubernetes` and `helm` providers with a
`data "aws_eks_cluster_auth"` token, as shown in
[`examples/small/providers.tf`](./examples/small/providers.tf). This is the
standard pattern for this module; use it in your own root module too:
...
An older, commonly documented alternative configures these providers with an
`exec` plugin that shells out to `aws eks get-token`. Avoid it: HCP
Terraform's hosted remote-run environment does not ship the AWS CLI, so any
remote plan or apply using `exec` wiring fails with `getting credentials:
...
between applies — for example, an HCP Terraform run role that is not the
same identity used for the first local apply — that identity gets
`Unauthorized` until it has its own EKS access entry. Grant it one with
...
module "n8n" {
  ...
  additional_access_entries = [
    "arn:aws:iam::123456789012:role/hcp-terraform-run-role",
  ]
}
```
...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_additional_access_entries"></a> [additional\_access\_entries](#input\_additional\_access\_entries) | IAM principal ARNs (users or roles) to grant cluster-admin access to the EKS cluster, beyond the identity that runs terraform apply. The cluster uses authenticationMode = "API" with bootstrap\_cluster\_creator\_admin\_permissions, so only the creating principal gets access automatically; every other identity needs its own aws\_eks\_access\_entry. The main use case is HCP Terraform: the run role that applies changes there is not a human's local AWS identity, so without an entry here every subsequent plan/apply from HCP Terraform gets Unauthorized against the Kubernetes API. Each ARN here receives a cluster-scoped access entry plus an AmazonEKSClusterAdminPolicy association. | `list(string)` | `[]` | no |
