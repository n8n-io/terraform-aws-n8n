...
Pin with `version = "~> 0.4.0"` to auto-receive 0.4.x patches without
accidentally crossing a 0.4 → 0.5 boundary. Note the three-component
constraint: `~> 0.4.0` resolves to `>= 0.4.0, < 0.5.0`, whereas the
two-component `~> 0.4` would resolve to `>= 0.4, < 1.0` and let you cross
minor boundaries unintentionally. To upgrade across minor lines, retype
the constraint (e.g. `version = "~> 0.5.0"`) and read the release notes.
...
v0.4.0 intentionally does not cover the following. Each item is
...
[`worker-pools`](./examples/worker-pools/) is the other topology variant at `small` sizing, apart from `node_max`, which it raises from 6 to 8 to hold the pools' own autoscaling ceilings. It runs three labelled worker pools beside the default worker deployment, each with its own replica bounds, sizing and KEDA scaler on its own `jobs-<name>` queue, so a project pinned to a pool runs only on that pool's workers. **Draft until upstream ships.** Worker pools are an alpha n8n feature: they need n8n `2.39.0` or later on the image, the `feat:workerPools` licence entitlement, and a Helm chart that renders `queueMode.workerGroups`, which no published chart version does yet ([n8n-io/n8n-hosting#189](https://github.com/n8n-io/n8n-hosting/pull/189)). A chart that predates it accepts the key and silently renders nothing, so the module warns at plan when `n8n_chart_version` predates the feature, the example makes `n8n_chart_version` a required input and shows how to push a preview build to ECR, and [`tests/scripts/verify-worker-pools.sh`](./tests/scripts/verify-worker-pools.sh) counts the rendered pools after apply.
