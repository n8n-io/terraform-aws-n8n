...
- **Terraform CLI `>= 1.9`**, with the `aws`, `kubernetes`, and `helm` providers configured by the caller. The module declares `required_providers` but does not configure them (see [`examples/small/providers.tf`](./examples/small/providers.tf)).
...
### Bumping `db_engine_version` across a major version

`aws_db_instance.n8n` sets `lifecycle { ignore_changes = [engine_version] }` so
that AWS's own automatic minor-version patching (`auto_minor_version_upgrade =
true`) doesn't produce a downgrade attempt on the next `terraform apply`. A
side effect: Terraform also ignores an *intentional* change to
`var.db_engine_version` on the instance itself, so simply bumping the variable
to a new major version (e.g. `"15.8"` to `"16.4"`) does not actually upgrade
the running instance.

`aws_db_parameter_group.n8n`'s `family` is derived from that same variable
(`postgres${split(".", var.db_engine_version)[0]}`) and is *not* covered by
`ignore_changes`, so it moves immediately. The result: bumping the major
version in your tfvars alone recreates the parameter group at the new family
while the instance stays on the old one, and RDS rejects the attachment as
incompatible.

To upgrade the major version safely: perform the actual engine upgrade
out-of-band first (the AWS console, `aws rds modify-db-instance
--engine-version <new> --allow-major-version-upgrade --apply-immediately`, or
a maintenance-window upgrade), wait for it to complete, and only then bump
`var.db_engine_version` to match. At that point the parameter group's new
family matches the instance's already-upgraded engine, and `terraform plan`
shows only the parameter-group swap.
...
| `redis_high_availability_enabled` | `redis_transit_encryption_enabled` | `redis_kms_encryption_enabled` | Resource | Nodes | Failover | TLS + AUTH | At-rest encryption |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `false` | `false` | `false` | `aws_elasticache_cluster` | 1 | no | no | **none** (`aws_elasticache_cluster` has no `at_rest_encryption_enabled` argument at all) |
| `true` | `false` | `false` | `aws_elasticache_replication_group` | 2, Multi-AZ | yes | no | AWS-managed |
| `false` | `true` | `false` | `aws_elasticache_replication_group` | 1 | no | yes | AWS-managed |
| `false` | `false` | `true` | `aws_elasticache_replication_group` | 1 | no | no | customer-managed |
| `true` | `true` | `false` | `aws_elasticache_replication_group` | 2, Multi-AZ | yes | yes | AWS-managed |
...
`redis_kms_encryption_enabled` defaults to `false`, but this is not "already
encrypted, upgrade optional" the way it is for RDS or S3: `at_rest_encryption_enabled`
is unconditional on the replication group (set the moment any of the three
toggles selects it), so `redis_high_availability_enabled` or
`redis_transit_encryption_enabled` already buys AWS-managed-key encryption as
a side effect, and a CMK on top of either is genuinely an upgrade over an
already-secure baseline. The default topology is the exception:
`aws_elasticache_cluster` has no `at_rest_encryption_enabled` argument at all,
so a deployment that leaves all three toggles alone is not encrypted at rest
by ElastiCache in any form, AWS-managed or otherwise. Setting
`redis_kms_encryption_enabled = true` on its own is the only one of the three
toggles that adds at-rest encryption where none previously existed, precisely
because doing so is what moves the deployment onto the replication group.

Because all three land on **one** resource with one identifier
(`<cluster_name>-redis-rg`), turning `redis_high_availability_enabled` or
`redis_transit_encryption_enabled` on *plans* as a modification of the
replication group you already have rather than a replacement, as long as at
least one of the three was already true. Turning on the first of the three is
what forces the initial replacement, whichever one that is.

`redis_kms_encryption_enabled` is the exception: `kms_key_id` on
`aws_elasticache_replication_group` is `ForceNew` (see `redis.tf`), so turning
it on or off *always* forces a full replacement of the replication group,
even when one already exists for HA or TLS alone. Flipping it on a live
deployment drops the queue for the duration of the replace, the same as
turning on the very first of the three; plan for a maintenance window rather
than expecting an in-place change.
...
`aws_kms_key.redis`, created only when `redis_kms_encryption_enabled = true`,
behaves the same way: `deletion_window_in_days = 7`, `aws_kms_alias.redis`
uses `name_prefix` for the same apply → destroy → apply reason, and recovery
is `aws kms cancel-key-deletion` followed by
`terraform import aws_kms_key.redis[0] <key-id>`.
...
| <a name="input_eks_secrets_encryption_enabled"></a> [eks\_secrets\_encryption\_enabled](#input\_eks\_secrets\_encryption\_enabled) | When true (the default), envelope-encrypt Kubernetes Secrets and the EKS control-plane CloudWatch log group with a module-created Customer Managed KMS Key (aws\_kms\_key.eks). Clears Checkov findings CKV\_AWS\_58 and CKV\_AWS\_158. Enabling this on an existing cluster may force EKS cluster replacement depending on the AWS provider version in use; test against a non-production cluster before flipping it on an existing deployment. Set to false to preserve current behavior on pre-existing clusters without secrets encryption. The CMK rotates annually and uses a 7-day deletion window (AWS minimum). | `bool` | `true` | no |
...
| <a name="input_redis_kms_encryption_enabled"></a> [redis\_kms\_encryption\_enabled](#input\_redis\_kms\_encryption\_enabled) | When true, encrypt Redis at rest with a module-created Customer Managed KMS Key (aws\_kms\_key.redis). Defaults to false, but this is not the usual "already encrypted, upgrade optional" default: at\_rest\_encryption\_enabled is unconditional on aws\_elasticache\_replication\_group, so redis\_high\_availability\_enabled or redis\_transit\_encryption\_enabled alone already get AWS-managed-key encryption for free, and a CMK on top of either is a genuine upgrade over an already-secure baseline. The default single-node aws\_elasticache\_cluster is the exception: that resource has no at\_rest\_encryption\_enabled argument at all, so a deployment that leaves every one of these three toggles alone is not encrypted at rest by ElastiCache in any form. kms\_key\_id only exists on aws\_elasticache\_replication\_group, so this is one of three variables (alongside redis\_high\_availability\_enabled and redis\_transit\_encryption\_enabled) that independently select the replication group, and the only one of the three that adds at-rest encryption where none previously existed: setting this true on a deployment that asks for neither HA nor TLS moves it off the single-node cluster onto the replication group, at the same one-node cost but a different resource type, specifically to gain that encryption. FLIPPING THIS ON AN EXISTING DEPLOYMENT REPLACES REDIS: kms\_key\_id is ForceNew, the same trap redis\_high\_availability\_enabled and redis\_transit\_encryption\_enabled document, so treat enabling it as a maintenance-window operation and drain the queue first. The CMK rotates annually and uses a 7-day deletion window (AWS minimum). Ignored when create\_elasticache = false. | `bool` | `false` | no |
...
| <a name="input_s3_kms_encryption_enabled"></a> [s3\_kms\_encryption\_enabled](#input\_s3\_kms\_encryption\_enabled) | When true (the default), set the S3 bucket's default encryption to SSE-KMS with a module-created Customer Managed KMS Key (aws\_kms\_key.s3) and grant the n8n pod role kms:Decrypt / kms:GenerateDataKey on it. Clears Checkov finding CKV\_AWS\_145. This selects which key encrypts objects, not whether they are encrypted: S3 encrypts every object regardless, and setting this to false leaves the bucket on SSE-S3 with S3-managed keys. S3 Bucket Keys are enabled alongside it so KMS is called per bucket rather than per object, which is what keeps request cost flat as execution volume grows. Two things to know before changing it on a live deployment: the setting applies to objects written after it, so existing objects keep the encryption they were written with, and flipping this from true to false destroys the CMK, which leaves every object written while it was true permanently undecryptable. The CMK rotates annually and uses a 7-day deletion window (AWS minimum). | `bool` | `true` | no |
