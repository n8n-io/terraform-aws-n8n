...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `false` | `false` | `false` | `aws_elasticache_cluster` | 1 | no | no | AWS-managed |
...
`redis_kms_encryption_enabled` defaults to `false`: ElastiCache already
encrypts every deployment at rest with an AWS-managed key regardless of this
variable (`at_rest_encryption_enabled` is unconditional), so a customer-managed
key is an upgrade over an already-secure baseline, not a gap the default needs
to close.
...
| <a name="input_redis_kms_encryption_enabled"></a> [redis\_kms\_encryption\_enabled](#input\_redis\_kms\_encryption\_enabled) | When true, encrypt Redis at rest with a module-created Customer Managed KMS Key (aws\_kms\_key.redis) instead of the AWS-managed key ElastiCache already uses by default. Defaults to false: at\_rest\_encryption\_enabled is unconditional in redis.tf, so every deployment is already encrypted at rest regardless of this variable, and a CMK on top of that is an upgrade over an already-secure baseline, not a gap to close. kms\_key\_id only exists on aws\_elasticache\_replication\_group, not on the default single-node aws\_elasticache\_cluster, so this is one of three variables (alongside redis\_high\_availability\_enabled and redis\_transit\_encryption\_enabled) that independently select the replication group: setting this true on a deployment that asks for neither HA nor TLS moves it off the single-node cluster onto the replication group, at the same one-node cost but a different resource type. FLIPPING THIS ON AN EXISTING DEPLOYMENT REPLACES REDIS: kms\_key\_id is ForceNew, the same trap redis\_high\_availability\_enabled and redis\_transit\_encryption\_enabled document, so treat enabling it as a maintenance-window operation and drain the queue first. The CMK rotates annually and uses a 7-day deletion window (AWS minimum). Ignored when create\_elasticache = false. | `bool` | `false` | no |
