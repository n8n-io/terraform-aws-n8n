...
| <a name="input_n8n_db_postgresdb_connection_timeout_ms"></a> [n8n\_db\_postgresdb\_connection\_timeout\_ms](#input\_n8n\_db\_postgresdb\_connection\_timeout\_ms) | Milliseconds a query waits for a database connection before failing<br/>(DB\_POSTGRESDB\_CONNECTION\_TIMEOUT). Leave null for n8n's own default of<br/>20000. Unsettable through the chart, which has no values path for it.<br/><br/>This is NOT the health-check timeout. A separate ping-timeout variable<br/>bounds n8n's periodic health check; this bounds every ordinary query's own<br/>connection acquisition, on a different code path, at request volume rather<br/>than ping volume. The two are independent and n8n's defaults happen to sit<br/>close enough that a saturated pool can cross both within minutes of each<br/>other, which makes the resulting failure hard to attribute.<br/><br/>Its name understates its reach. n8n passes it to pg-pool as<br/>connectionTimeoutMillis, and the comment in db-connection-options.ts<br/>records the overloading deliberately: it bounds both establishing a new<br/>connection and "an in-use connection to free a slot", reusing "the<br/>existing connect-timeout knob rather than adding a new env var". So<br/>raising it to tolerate a slow handshake also lengthens how long requests<br/>queue behind a busy pool, and lowering it to fail fast on an unreachable<br/>database also reduces how much local queueing a pod tolerates.<br/><br/>Raising this is not a fix for pool saturation. Measured on a live<br/>deployment: acquire time climbed smoothly past 25 seconds over a two-hour<br/>run, so any fixed value is crossed eventually and only the timing moves.<br/>Reach for pool size, pod count, or per-pod request concurrency instead.<br/>Exposed here because the knob governing the failure should be settable,<br/>including downward for fail-fast behaviour, not because raising it is<br/>advisable. | `number` | `null` | no |
