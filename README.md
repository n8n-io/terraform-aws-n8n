...
[`worker-pools`](./examples/worker-pools/) is the other topology variant at `small` sizing, apart from `node_max`, which it raises from 6 to 8 to hold the pools' own autoscaling ceilings. It runs three labelled worker pools beside the default worker deployment, each with its own replica bounds, sizing and KEDA scaler on its own `jobs-<name>` queue, so a project pinned to a pool runs only on that pool's workers. Worker pools are an alpha n8n feature and need a licence entitlement; see that example's README.
