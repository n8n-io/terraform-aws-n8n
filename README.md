...
| <a name="input_n8n_worker_keda_min_replicas"></a> [n8n\_worker\_keda\_min\_replicas](#input\_n8n\_worker\_keda\_min\_replicas) | Minimum worker replicas. KEDA keeps at least this many workers running even when the queue is empty, including 0 (KEDA scales the worker Deployment to zero natively). The chart's own queueMode.workerReplicaCount is floored at 1 by n8n.tf regardless of this value, so the worker Deployment and its ScaledObject always render; this value only feeds keda.worker.minReplicaCount, KEDA's own floor. Chart >= 1.13.0 leaves the worker Deployment's replica count to the autoscaler once it owns it (the module always sets non-empty keda.worker.triggers), so the workerReplicaCount floor only sets spec.replicas directly on a chart older than 1.13.0. On a chart older than 1.13.0, the floored value is written as spec.replicas on every Helm upgrade, so an upgrade can reset a larger live worker count down to it and KEDA scales back above it only when queue demand requires it. | `number` | `1` | no |
| <a name="input_n8n_worker_keda_pause"></a> [n8n\_worker\_keda\_pause](#input\_n8n\_worker\_keda\_pause) | Pause KEDA autoscaling of the worker Deployment (sets autoscaling.keda.sh/paused on the worker ScaledObject, chart's keda.worker.pause, n8n-io/n8n-hosting#177). While paused, KEDA stops reconciling and workers hold their current replica count, or n8n\_worker\_keda\_paused\_replica\_count when that is also set. Use for a maintenance window or to drain the queue ahead of a migration without disabling KEDA outright. Applies to the chart's default worker Deployment only: n8n\_worker\_pools pools have their own ScaledObjects and keep scaling on their own queues. Requires chart 1.13.0 or newer (the module default); a plan-time warning fires otherwise. The worker Deployment and its ScaledObject always render regardless of n8n\_worker\_keda\_min\_replicas, since n8n.tf floors queueMode.workerReplicaCount at 1 independently of that value, so there is always a ScaledObject for these annotations to land on. The key shipped in chart 1.12.0, but that release still sets the worker's spec.replicas on every Helm upgrade, so a later apply while paused can write the floor back over the held count. | `bool` | `false` | no |
