...
These are module inputs, so overriding them no longer means wrapping or forking the module. This example does not pass them through as its own variables: set them directly on the `module "n8n"` block in `main.tf`, or copy the pass-through variables from [`examples/small`](../small/) if you want them in `terraform.tfvars`.
