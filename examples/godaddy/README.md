...
`godaddy-dns_record.cert_validation` from `toset([var.n8n_domain])` to the
...
| <a name="input_n8n_domain"></a> [n8n\_domain](#input\_n8n\_domain) | Fully-qualified domain name for n8n (e.g. n8n.example.com). Must be a single label below godaddy\_domain (e.g. 'n8n' below 'example.com'). Deeper nesting like 'n8n.prod.example.com' under 'example.com' is not supported by this example's name-stripping logic — host such records under godaddy\_domain = 'prod.example.com' instead. Must be 64 characters or fewer: this example uses it as the Common Name of the ACM certificate it issues, which RFC 5280 caps at 64 octets. | `string` | n/a | yes |
