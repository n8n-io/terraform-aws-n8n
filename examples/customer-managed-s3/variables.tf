...
# Describes the aws_s3_bucket this example creates to play the part of a
# customer's already-existing bucket. In a real deployment this resource
# would not exist here at all: the bucket would already be running, secured
# however its owner already configured it, and you would only need its name
# to fill in the module's existing_s3_bucket_name input directly. See
