...
# The medium example passes these through to the module unchanged. Verify the
...
run "deletion_controls_pass_through_to_module" {
  ...
  variables {
    ...
    db_final_snapshot_identifier = "medium-example-final"
  ...
  assert {
    condition     = module.n8n.rds_final_snapshot_identifier == "medium-example-final"
