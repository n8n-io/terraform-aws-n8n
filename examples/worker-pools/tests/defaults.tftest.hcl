...
# The worker-pools example passes these through to the module unchanged, the
# same way examples/small does. Verify the wiring by reading the module outputs
# at their defaults and again after setting non-default values.
...
run "deletion_controls_pass_through_to_module" {
  ...
  variables {
    ...
    db_final_snapshot_identifier = "worker-pools-example-final"
  ...
  assert {
    condition     = module.n8n.rds_final_snapshot_identifier == "worker-pools-example-final"
