...
variable "route53_zone_id" {
  description = "Route53 hosted zone ID for the parent of n8n_domain (e.g. the zone for example.com if n8n_domain = n8n.example.com). The module creates the ACM certificate and validation records inside this zone; this example's own dns.tf creates the alias A-record, since create_ingress = false means the module owns no load balancer to point one at."
  type        = string
}
...
variable "n8n_image_tag" {
  description = "n8n application image tag to deploy (e.g. \"2.27.4\"). Leave null to use the selected Helm chart's default (appVersion 2.39.6 in chart 1.12.0). Inspect and pin the running application version before upgrading the chart to avoid a downgrade; see docs/upgrading-n8n.md."
  type        = string
  default     = null
...
# Same variables, same defaults, as examples/customer-managed-cluster. See
# that example's README for the full reasoning behind each default.
...
# Describes the aws_db_instance this example creates to play the part of a
# customer's already-existing database. In a real deployment this section
# would not exist here at all: the database would already be running, and
# you would only need its endpoint and password to fill in the module's
# db_host / db_password inputs directly. See "Adapting to your real
# infrastructure" in this example's README.

variable "customer_managed_db_instance_class" {
...
variable "customer_managed_db_password" {
  description = "Master password for the stand-in database this example creates. Deliberately a plain variable, not a generated random_password: this value is wired both into the stand-in's own password argument and into the module's db_password, and a fresh random_password.result is unknown until apply, which this example's Redis stand-in already ran into once (see customer_managed_redis_auth_token below and examples/customer-managed-redis's README for the full explanation of why that breaks a plan). A real customer-managed database's password is already a known secret the caller holds, not something Terraform generates in the same apply. The default below is fine for a disposable demo stack; generate and manage your own for anything that outlives one terraform destroy."
  type        = string
  sensitive   = true
...
# Same variables, same defaults and same reasoning, as
# examples/customer-managed-redis. See that example's README for the full
# writeup of why the AUTH token is a plain variable rather than a generated
# secret.
...
# Same variable, same default and same reasoning, as
# examples/customer-managed-s3.

variable "customer_managed_s3_force_destroy" {
