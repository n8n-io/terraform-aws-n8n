...
# terraform destroy then orphans it, confirmed live on a destroy of
# examples/customer-managed-cluster, whose stand-in this one mirrors. The
# module itself deliberately does not manage this group on the create_eks =
# false path (an existing cluster's log group belongs to whoever manages the
# cluster), and in this example that is this file.
