...
run "mcp_single_replica_pins_webhook_processors" {
  command = plan
  ...
  # MCP sessions live in one replica's memory; more than one replica means
  # intermittent 404 Session not found. Asserted on the example's own wiring
  # because helm_release values are unknown at plan time under mocks.
  assert {
    condition     = local.webhook_hpa_min_replicas == 1
    error_message = "mcp_single_replica = true must floor the webhook HPA at 1 replica"
  }

  # The ceiling matters more than the floor. A max above 1 lets the HPA scale
  # out under load and silently reintroduces the lost-session failure at
  # exactly the moment traffic is highest.
  assert {
    condition     = local.webhook_hpa_max_replicas == 1
    error_message = "mcp_single_replica = true must also cap the webhook HPA at 1 replica"
  }
}

run "mcp_single_replica_off_allows_webhook_scale_out" {
  command = plan

  variables {
    mcp_single_replica = false
  }

  # The default path must not inherit the MCP pin: webhook throughput is the
  # reason the processors are a separate Deployment in the first place.
  assert {
    condition     = local.webhook_hpa_max_replicas > 1
    error_message = "mcp_single_replica = false must leave the webhook HPA free to scale out"
  }
}
