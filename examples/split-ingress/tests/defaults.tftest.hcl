...
# The split-ingress example passes these through to the module unchanged, the same way
...
run "deletion_controls_pass_through_to_module" {
  ...
  variables {
    ...
    db_final_snapshot_identifier = "split-ingress-example-final"
  ...
  assert {
    condition     = module.n8n.rds_final_snapshot_identifier == "split-ingress-example-final"
