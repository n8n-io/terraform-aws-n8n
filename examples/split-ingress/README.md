...
This example is a reference deployment optimized for clean `apply` / `destroy` cycles during evaluation. Review these before promoting it. Every "`variables.tf`" row is this example's own variable, settable in `terraform.tfvars` as usual; the deletion-control rows are passed straight through to the module.

| Where | Setting | Current | Production |
| --- | --- | --- | --- |
| `main.tf` (VPC) | `single_nat_gateway` | `true` | `false` + `one_nat_gateway_per_az = true` |
| `variables.tf` | `db_deletion_protection` | `false` | `true` |
| `variables.tf` | `db_skip_final_snapshot` | `true` | `false`, plus set `db_final_snapshot_identifier` |
| `variables.tf` | `db_delete_automated_backups` | `true` | `false` |
| `variables.tf` | `s3_force_destroy` | `true` | `false` |
...
| `variables.tf` | `db_backup_retention_period` | `7` | Match your RPO |
