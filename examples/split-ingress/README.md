...
Set `mcp_single_replica = true` to pin the webhook processors to one replica. That makes MCP reliable at the cost of webhook throughput and HA. That is a real trade, so it defaults to `false`. Tracked in [issue #59](https://github.com/n8n-io/terraform-aws-n8n/issues/59); see also [docs/troubleshooting.md](../../docs/troubleshooting.md#mcp-server-trigger-returns-404-session-not-found-intermittently).
