...
run "redis_defaults_to_a_single_node_cluster" {
  ...
  assert {
    ...
    error_message = "redis_snapshot_retention_limit must default to 1 daily snapshot"
  }
...
run "redis_high_availability_creates_a_failover_capable_replication_group" {
  ...
  assert {
    ...
    error_message = "redis_snapshot_retention_limit must default to 1 daily snapshot on the replication group too, independent of the single-node cluster path"
  }
...
# The rule above is genuinely conditional on the public endpoint being on: a
# private-only caller who passes [] to make that explicit (matching the
# variable's own "Ignored when cluster_endpoint_public_access = false"
# description) must not be blocked by a check that only matters for the
# public path.
