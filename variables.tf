...
variable "n8n_main_hpa_min_replicas" {
  description = "Minimum replicas for n8n main pods. HPA will not scale below this. Also becomes the deployment's own replica count: the Helm chart renders spec.replicas unconditionally, so leaving it below the autoscaler floor would make every helm upgrade scale down and then wait for the autoscaler to climb back. At 2 or more, multi-main is enabled and requires feat:multipleMainInstances; the main PodDisruptionBudget protects one available main during voluntary eviction. Set to 1 for Business-compatible single-main queue mode: multi-main is disabled, the HPA maximum is clamped to 1 regardless of n8n_main_hpa_max_replicas, upgrades use Recreate, and the disruption budget allows eviction with downtime. The editor, REST API, and scheduled triggers are unavailable while the only main is stopped. Recreate prevents upgrade overlap, not overlap following manual pod deletion or node failure. See docs/upgrading-n8n.md."
  type        = number
  default     = 2
  nullable    = false
...
variable "n8n_worker_keda_min_replicas" {
  description = "Minimum worker replicas. KEDA keeps at least this many workers running even when the queue is empty, including 0 (KEDA scales the worker Deployment to zero natively). The chart's own queueMode.workerReplicaCount is floored at 1 by n8n.tf regardless of this value, so the worker Deployment and its ScaledObject always render; this value only feeds keda.worker.minReplicaCount, KEDA's own floor. Chart >= 1.13.0 leaves the worker Deployment's replica count to the autoscaler once it owns it (the module always sets non-empty keda.worker.triggers), so the workerReplicaCount floor only sets spec.replicas directly on a chart older than 1.13.0. On a chart older than 1.13.0, the floored value is written as spec.replicas on every Helm upgrade, so an upgrade can reset a larger live worker count down to it and KEDA scales back above it only when queue demand requires it."
  type        = number
  default     = 1
  nullable    = false
  ...
  validation {
    ...
    error_message = "n8n_worker_keda_min_replicas must be a whole number of replicas, 0 or greater. 0 is allowed here, unlike the two HPA floors: KEDA scales the worker Deployment to zero natively, and the chart still renders the worker Deployment and ScaledObject because n8n.tf floors queueMode.workerReplicaCount at 1 independently of this value."
  }
}
